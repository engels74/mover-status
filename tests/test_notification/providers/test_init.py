...
class TestProviderPackageDiscovery:
    ...
    def test_discover_provider_packages(self) -> None:
        ...
        with patch('os.listdir') as mock_listdir, \
            ...
            mock_isdir.side_effect = lambda path: not path.endswith('__pycache__')
    ...
    def test_register_discovered_providers(self) -> None:
        ...
        with patch('mover_status.notification.providers.load_provider_class') as mock_load:
            mock_load.side_effect = lambda name: provider_classes.get(name)
    ...
    def test_handle_missing_or_invalid_provider_packages(self) -> None:
        ...
        with patch('importlib.import_module') as mock_import:
            mock_module = Mock()
            mock_module.get_provider_class.return_value = "not_a_class"
            mock_import.return_value = mock_module
    ...
    def test_load_provider_class_success(self) -> None:
        ...
        with patch('importlib.import_module') as mock_import:
            mock_module = Mock()
            mock_module.get_provider_class.return_value = MockProvider
            mock_import.return_value = mock_module
    ...
    def test_provider_package_validation(self) -> None:
        ...
        with patch('os.path.exists') as mock_exists:
            mock_exists.side_effect = lambda path: path.endswith('__init__.py') or path.endswith('provider.py')
        ...
        with patch('os.path.exists') as mock_exists:
            mock_exists.side_effect = lambda path: path.endswith('provider.py')  # Only provider.py exists
        ...
        with patch('os.path.exists') as mock_exists:
            mock_exists.side_effect = lambda path: path.endswith('__init__.py')  # Only __init__.py exists
