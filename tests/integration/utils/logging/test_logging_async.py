...
class TestAsyncPerformance:
    ...
    async def test_async_logging_overhead(self) -> None:
        ...
        # Overhead should be reasonable (less than 200%)
        assert logging_overhead < 2.0, f"Logging overhead too high: {logging_overhead:.1%}"
        assert context_overhead < 2.0, f"Context overhead too high: {context_overhead:.1%}"
