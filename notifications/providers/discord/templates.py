...
from shared.types.discord import ApiLimit
...
def create_footer(version_info: Optional[str] = None) -> EmbedFooter:
    ...
    return {
        "text": truncate_string(footer_text, ApiLimit.FOOTER_TEXT_LENGTH)
    }
...
def create_progress_field(
    ...
    return {
        "name": truncate_string(name, ApiLimit.FIELD_NAME_LENGTH),
        "value": truncate_string(value, ApiLimit.FIELD_VALUE_LENGTH),
        "inline": False
    }
...
def create_progress_embed(
    ...
    """
    embed: Embed = {
        "title": truncate_string(title, ApiLimit.TITLE_LENGTH),
    ...
    if description:
        embed["description"] = truncate_string(
            description,
            ApiLimit.DESCRIPTION_LENGTH
        )

    if author:
        embed["author"] = {
            "name": truncate_string(author["name"], ApiLimit.AUTHOR_NAME_LENGTH),
            "url": author.get("url"),
...
def create_completion_embed(
    ...
    if stats:
        ...
        for key, value in stats.items():
            fields.append({
                "name": truncate_string(str(key), ApiLimit.FIELD_NAME_LENGTH),
                "value": truncate_string(str(value), ApiLimit.FIELD_VALUE_LENGTH),
                "inline": True
            })
        if fields:
            embed["fields"] = fields
...
def create_error_embed(
    ...
    embed: Embed = {
        ...
        "description": truncate_string(description, ApiLimit.DESCRIPTION_LENGTH),
    ...
    if error_details:
        ...
        for key, value in error_details.items():
            fields.append({
                "name": truncate_string(key, ApiLimit.FIELD_NAME_LENGTH),
                "value": truncate_string(value, ApiLimit.FIELD_VALUE_LENGTH),
                "inline": True
            })
        if fields:
...
def validate_title(title: Optional[str]) -> int:
    ...
    if not title:
        return 0
    if len(title) > ApiLimit.TITLE_LENGTH:
        raise ValueError(f"Title exceeds {ApiLimit.TITLE_LENGTH} characters")
    return len(title)
...
def validate_description(description: Optional[str]) -> int:
    ...
    if not description:
        return 0
    if len(description) > ApiLimit.DESCRIPTION_LENGTH:
        raise ValueError(f"Description exceeds {ApiLimit.DESCRIPTION_LENGTH} characters")
...
def validate_fields(fields: Optional[List[EmbedField]]) -> int:
    ...
    total_length = 0
    for field in fields:
        if len(field["name"]) > ApiLimit.FIELD_NAME_LENGTH:
            raise ValueError(f"Field name exceeds {ApiLimit.FIELD_NAME_LENGTH} characters")
        if len(field["value"]) > ApiLimit.FIELD_VALUE_LENGTH:
            raise ValueError(f"Field value exceeds {ApiLimit.FIELD_VALUE_LENGTH} characters")
...
def validate_footer(footer: Optional[Dict[str, str]]) -> int:
    ...
    if not footer or "text" not in footer:
        return 0
    if len(footer["text"]) > ApiLimit.FOOTER_TEXT_LENGTH:
        raise ValueError(f"Footer text exceeds {ApiLimit.FOOTER_TEXT_LENGTH} characters")
...
def validate_author(author: Optional[Dict[str, str]]) -> int:
    ...
    if not author or "name" not in author:
        return 0
    if len(author["name"]) > ApiLimit.AUTHOR_NAME_LENGTH:
        raise ValueError(f"Author name exceeds {ApiLimit.AUTHOR_NAME_LENGTH} characters")
...
def create_webhook_data(
    ...
    Args:
        ...
        thread_name: Optional thread name for forum posts
    ...
    """
    if len(embeds) > ApiLimit.EMBEDS_PER_MESSAGE:
        raise ValueError(f"Maximum of {ApiLimit.EMBEDS_PER_MESSAGE} embeds allowed")

    for embed in embeds:
        validate_embed_lengths(embed)

    payload: WebhookPayload = {
        "username": truncate_string(username, ApiLimit.USERNAME_LENGTH),
        "embeds": embeds
    }
    ...
    if thread_name:
        payload["thread_name"] = truncate_string(thread_name, ApiLimit.CHANNEL_NAME_LENGTH)
